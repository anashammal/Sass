...
<div class="col-md-4">
    <label class="form-label fw-bold text-dark">تاريخ الفاتورة</label>
    <div class="input-group shadow-sm">
        <span class="input-group-text bg-primary text-white"><i class="fas fa-calendar-alt"></i></span>
        {{-- حقل نصي عادي سيحوله السكربت لتاريخ احترافي --}}
        <input type="text" id="invoice_date_picker" name="invoice_date" 
               class="form-control fw-bold text-primary text-center"
               style="font-family: sans-serif; font-size: 1.1rem; letter-spacing: 1px;"
               value="{{ \Carbon\Carbon::now()->addHours(3)->format('Y-m-d H:i') }}" required>
    </div>
</div>
                            <div class="col-md-4">
                                <label class="form-label">رقم الفاتورة (تلقائي)</label>
                                {{-- 🔥 وضع الرقم التلقائي وجعله للقراءة فقط (اختياري) 🔥 --}}
                                <input type="text" name="invoice_number" class="form-control bg-light fw-bold text-primary" 
...
@endsection
{{-- مكتبة التاريخ والوقت الاحترافية --}}
...
<script>
    ...
    // --- عند تحميل الصفحة (تشغيل البحث) ---
    document.addEventListener("DOMContentLoaded", function() {
        // 1. إعداد بحث الموردين (استخدمنا url لضمان صحة الرابط)
    ...
    // --- دالة إعداد البحث (المصححة والنهائية) ---
                ...
                fetch(`${url}?term=${term}`)
                    ...
                    .then(data => {
                        ...
                        if (data.length > 0) {
                            ...
                            // 🔥 الاختيار التلقائي الذكي (إذا نتيجة واحدة مطابقة تماماً) 🔥
                            if (data.length === 1 && inputId === 'productSearch') {
                                let item = data[0];
                                // هل المدخل مطابق للباركود أو الـ SKU؟
                                let isExact = (item.units && item.units.some(u => u.barcode == term)) || item.sku == term;
                                if (isExact) {
                                    onSelect(item);
                                    results.style.display = 'none';
                                    input.value = '';
                            ...
                            data.forEach((item, index) => {
                                ...
                                a.addEventListener("click", function(e) {
                                    ...
                                    onSelect(item); 
                                    results.style.display = 'none';
                                    if(inputId !== 'supplierSearchInput') input.value = '';
                                });
    ...
    // --- باقي الدوال (حسابات، صور، إضافة صف) ---
    // تأكد من نسخ باقي الدوال التي كانت موجودة (addProductRow, syncSubUnits, etc.)
    // سأضع لك الدوال الأساسية هنا لضمان العمل
    ...
    // --- دالة إضافة صف المنتج (مع التصحيح الكامل) ---
    ...
    // --- دالة المزامنة الرياضية (محدثة: تحدث الربح فوراً) ---
    function syncSubUnits(idx) {
        calcTotals(idx);
        let row = document.getElementById(`row_${idx}`);
        
        let mainCost = parseFloat(row.querySelector('.price').value) || 0; 
        let mainSell = parseFloat(row.querySelector('.sell').value) || 0;

        if (mainCost > 0) {
            let mainProfit = ((mainSell - mainCost) / mainCost) * 100;
            row.querySelector('.profit').value = formatNum(mainProfit);
        } else {
            row.querySelector('.profit').value = 0;
        }
        ...
        let sellPerPiece = 0;
        if (currentFactor > 0) {
            costPerPiece = mainCost / currentFactor;
    ...
    // --- باقي دوال الحسابات (calcSellPrice, calcProfitPercent...) ---
    // تأكد من نسخهم كما أعطيتك في الرد السابق، أو إذا أردت سأضعهم لك هنا للاطمئنان
    ...
    function calcProfitPercent(idx) {
        ...
        if(mainCost > 0) {
            let profit = ((mainSell - mainCost) / mainCost) * 100;
            row.querySelector('.profit').value = formatNum(profit);
        }

        // تحديث بيع الوحدات الفرعية
        let select = row.querySelector('.unit-select');
        let selectedOption = select.options[select.selectedIndex];
        let currentFactor = parseFloat(selectedOption.getAttribute('data-factor')) || 1;
        ...
        if(container) {
            container.querySelectorAll('.related-unit-row').forEach(subRow => {
                let subFactor = parseFloat(subRow.getAttribute('data-factor')) || 1;
                let subCost = parseFloat(subRow.querySelector('.sub-cost').value) || 0;
                ...
                let newSubProfit = 0;
                if (subCost > 0) {
                    newSubProfit = ((newSubSell - subCost) / subCost) * 100;
                }
                subRow.querySelector('.sub-profit').value = formatNum(newSubProfit);
            });
        }
